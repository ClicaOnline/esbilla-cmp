...
- **Key Endpoints**:
  ...
  - **GTM Gateway Proxy (v1.8+):**
    - `GET /gtm.js` - Multi-tenant GTM proxy (DNS-based, cached, compressed)
    - `GET /metrics/:check` - Health check for GTM Gateway

### GTM Gateway Proxy (v1.8+ - Add-on Premium)

**Architecture:** Multi-tenant DNS-based proxy with modular scalability

**Purpose:** Proxy Google Tag Manager scripts through client's own domain to evade ad blockers and improve performance.

**Flow:**
```
Cliente → gtm.cliente.com (DNS → Esbilla API) → Google GTM → Cache → Compresión → Cliente
```

**Key Features:**
- **Multi-tenant routing:** Host header identifies client (gtmGatewayDomain → siteId → containerId)
- **Cache:** In-memory cache per Cloud Run instance (TTL 5 min) + Cloud CDN global cache
- **Compression:** Brotli/Gzip reduces GTM script size 75% (80 KB → 20 KB)
- **Geolocation:** X-Forwarded-Country-Region headers for Google targeting
- **Rate limiting:** 10 req/min per IP for GTM endpoint
- **GDPR compliance:** All regions in EU (no data transfer outside EU)

**Infrastructure (Modular):**
1. **Cloud CDN:** Global cache in EU PoPs (Frankfurt, London, Paris, etc.)
2. **Load Balancer:** Multi-region distribution (europe-west4, europe-west1, europe-west3)
3. **Cloud Run:** Auto-scaling compute (1-100 instances per region)
4. **Firestore:** Configuration lookup (gtmGatewayDomain → containerId)
5. **Google GTM:** External source (googletagmanager.com, fps.goog)

**Scalability Strategy:**
- **Horizontal:** Add more Cloud Run instances or regions
- **Vertical:** Increase CPU/memory per instance
- **CDN:** Add more PoPs for geographic distribution
- **Multi-region:** All in EU for GDPR (europe-west4, west1, west3, north1)

**Configuration (Dashboard):**
- `gtmGatewayEnabled`: Enable/disable proxy
- `gtmGatewayDomain`: Client's custom domain (e.g., gtm.cliente.com)
- `gtmContainerId`: GTM-XXXXX or G-XXXXX

**DNS Setup (Client):**
- CNAME record: `gtm.cliente.com → api.esbilla.com`
- Or A record pointing to Load Balancer IP

**Cost Impact:**
- Base: +5-15% egress bandwidth (~€1.50/mes per 1M PV with optimizations)
- With CDN: 92% reduction in Cloud Run egress (cache hit rate >80%)
- Pricing: Add-on premium +€10-30/mes depending on volume

**Documentation:**
- [docs/GTM-GATEWAY-SETUP.md](docs/GTM-GATEWAY-SETUP.md) - Configuration guide
- [docs/GTM-GATEWAY-PROXY-COSTS.md](docs/GTM-GATEWAY-PROXY-COSTS.md) - Cost analysis
- [docs/GTM-GATEWAY-INFRASTRUCTURE.md](docs/GTM-GATEWAY-INFRASTRUCTURE.md) - Infrastructure and scalability

**Key Code Locations:**
- [esbilla-api/src/app.js](esbilla-api/src/app.js) (lines 1110-1314) - GTM proxy endpoint with multi-tenant routing
- [esbilla-api/public/pegoyu.js](esbilla-api/public/pegoyu.js) (lines 748-800) - SDK GTM loading with custom domain support
- [esbilla-dashboard/src/pages/Sites.tsx](esbilla-dashboard/src/pages/Sites.tsx) (lines 829-950) - Dashboard UI for GTM Gateway config
